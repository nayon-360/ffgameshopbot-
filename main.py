...
# Encode credentials
db_user = "ffgameshop"
db_password = "@gg#$123Dg"
encoded_user = quote_plus(db_user)
encoded_pass = quote_plus(db_password)

# Mongo URI
MONGO_URI = f"mongodb+srv://{encoded_user}:{encoded_pass}@ffgameshop.srngocw.mongodb.net/?retryWrites=true&w=majority&appName=ffgameshop"

# Connect to MongoDB
mongo_client = MongoClient(MONGO_URI)
db = mongo_client["telegram_bot_db"]  # ✅ MAKE SURE THIS LINE COMES FIRST

# Now collections
users_collection = db["users"]
# ... other collections follow here
...
async def bkash_pay(event):
    ...
    if not is_user_signed_up(event.sender_id):
        await event.reply("➥ **You need to sign up first! Use Ssignup**")
        return
    ...
    try:
        ...
        # বর্তমান ব্যালেন্স এবং ডিউ
        current_balance = users.get(user_id, {}).get("balance", 0)
        current_due = baki_data.get(user_id, {}).get("due", 0)

        # bKash পেমেন্ট তৈরি (ফি বাদ দেওয়া হয়েছে, শুধু মূল টাকা)
        payment_response = create_bkash_payment(amount, user_id)
        ...
        # ফরম্যাট অনুযায়ী রেসপন্স, ফি-সংক্রান্ত লাইন বাদ দেওয়া হয়েছে
        response = (
            "▔▔▔▔▔▔▔▔▔▔▔▔\n"
            f"➪ Nᴀᴍᴇ        : {display_name}\n"
